...
it('correctly renders/does not render the add task follower button', () => {
  ...
  const withButton = new AddFollower({
    patientId: patient.id,
    taskId: task.id,
    followers: [user1],
    careTeam: [user1, user2],
  });
  const withoutButton = new AddFollower({
    patientId: patient.id,
    taskId: task.id,
    followers: [user1, user2],
    careTeam: [user1, user2],
  ...
  expect(withButton.getValidNewFollowers()).toEqual([user2]);
  expect(withoutButton.getValidNewFollowers()).toEqual([]);
});
