...
describe('user model', () => {
  ...
  it('retrieve user by email', async () => {
    const clinic = await Clinic.create(createMockClinic(), txn);
    const user = await User.create(createMockUser(11, clinic.id, userRole, 'danplant@b.com'), txn);
  ...
  it('marks a user as deleted', async () => {
    ...
    const user2 = await User.create(createMockUser(1, clinic2.id, userRole, 'user@place.com'), txn);
