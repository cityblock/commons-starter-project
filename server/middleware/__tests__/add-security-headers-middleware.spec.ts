...
describe('addHeadersMiddleware', () => {
  it('adds security headers', async () => {
    ...
    expect(response.getHeader('Content-Security-Policy')).toEqual(
      "default-src 'self' https://accounts.google.com blob:; script-src 'self' *.google.com  *.google-analytics.com unpkg.com; style-src 'self' https://fonts.googleapis.com 'unsafe-inline' blob:; font-src 'self' https://fonts.gstatic.com data:; img-src 'self' *.googleusercontent.com *.google-analytics.com https://storage.googleapis.com data: blob:; connect-src 'self' https://storage.googleapis.com ws://localhost:3000/subscriptions; frame-ancestors 'self'",
    );
