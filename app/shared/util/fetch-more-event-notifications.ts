import { getEventNotificationsForCurrentUserQuery } from '../../graphql/types';

export type EventNotificationsResponse = getEventNotificationsForCurrentUserQuery & {
  fetchMore: any;
};
export type Key = 'eventNotificationsForCurrentUser';

function updateQuery(
  previousResponse: EventNotificationsResponse,
  fetchMoreResponse: EventNotificationsResponse,
  key: Key,
) {
  const result = fetchMoreResponse[key];

  if (!result) {
    return previousResponse;
  }

  const newEdges = result.edges || [];
  const pageInfo = result.pageInfo;

  const previousResult = previousResponse[key];
  const previousEdges = previousResult && previousResult.edges ? previousResult.edges : [];

  const response: any = {};
  response[key] = {
    edges: [...previousEdges, ...newEdges],
    pageInfo,
  };
  return response;
}

export function fetchMoreEventNotifications(
  data: EventNotificationsResponse,
  variables: any,
  key: Key,
) {
  const result = data[key];
  if (!data.fetchMore || !result) {
    return;
  }

  // auto-add pageNumber based on results
  variables.pageNumber = result.edges ? Math.floor(result.edges.length / 15) : 0;

  return data.fetchMore({
    variables,
    updateQuery: (previousResult: EventNotificationsResponse, d: any) =>
      updateQuery(previousResult, d.fetchMoreResult, key),
  });
}
