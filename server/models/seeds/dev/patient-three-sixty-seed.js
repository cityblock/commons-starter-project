...
function createRadioQuestion(knex, riskAreaId, patientIds) {
  return knex
    .table('question')
    .returning('id')
    .insert(buildQuestion('Where have you lived in the past 2 months?', 1, riskAreaId, 'radio'))
      ...
      return knex
        ...
        .then(function(answerIds) {
          ...
          for (var i = 0; i < patientIds.length; i++) {
            patientAnswers.push(buildPatientAnswer(answerIds[1], patientIds[i]));
          }
...
function createMultiselectQuestion(knex, riskAreaId, patientIds) {
  return knex
    .table('question')
    .returning('id')
    .insert(buildQuestion('Who do you live with?', 2, riskAreaId, 'multiselect'))
    .then(function(questionIds) {
      return knex
        .table('answer')
        .returning('id')
...
function createFreetextQuestion(knex, riskAreaId, patientIds) {
  return knex
    .table('question')
    .returning('id')
    .insert(buildQuestion('What is your current address?', 4, riskAreaId, 'freetext'))
    .then(function(questionIds) {
      return knex
        .table('answer')
        .returning('id')
        ...
        .then(function(answerIds) {
          ...
          for (var i = 0; i < patientIds.length; i++) {
            patientAnswers.push(buildPatientAnswer(answerIds[0], patientIds[i], 'Winterfell'));
          }
          return knex.table('patient_answer').insert(patientAnswers);
        });
    });
...
function createDiabetesQuestion(knex, riskAreaId, patientIds, computedFieldId) {
  return knex
    .table('question')
    .returning('id')
    .insert(buildQuestion('Pharmacy claim for insulin?', 1, riskAreaId, 'radio', computedFieldId))
    .then(function(questionIds) {
      return knex
        .table('answer')
        .returning('id')
        ...
        .then(function(answerIds) {
          ...
          for (var i = 0; i < patientIds.length; i++) {
            patientAnswers.push(buildPatientAnswer(answerIds[0], patientIds[i]));
          }
          return knex.table('patient_answer').insert(patientAnswers);
        });
    });
...
function createRiskAreaGroupPromise(knex, Promise, riskAreaGroupTitle) {
  return knex
    .table('risk_area_group')
    .returning('id')
    .insert(buildRiskAreaGroup(riskAreaGroupTitle))
    .then(function(riskAreaGroupIds) {
      return knex
        ...
        .insert([
          ...
          buildRiskArea(
            `${riskAreaGroupTitle} - Congestive Heart Failure`,
            4,
            riskAreaGroupIds[0],
            'automated',
          ),
        ...
        .then(function(riskAreaIds) {
          return knex
            .table('computed_field')
            .pluck('id')
            .then(function(computedFieldIds) {
