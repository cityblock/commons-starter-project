...
import * as getPatientMedications from '../../../app/graphql/queries/get-patient-medications.graphql';
...
import {
  createMockClinic,
  createMockPatientMedicationsFile,
  createMockUser,
  createPatient,
...
describe('patient problem list tests', () => {
  ...
  const log = jest.fn();
  const logger = { log };
  const patientMedicationsQuery = print(getPatientMedications);
  ...
  describe('medications for patient', () => {
    ...
    it('returns an empty medication list for a patient', async () => {
      ...
      expect(cloneDeep(result.data!.patientMedications)).toEqual([]);
    });
  });
});
