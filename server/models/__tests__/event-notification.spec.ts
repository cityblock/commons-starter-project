...
describe('task event model', () => {
  ...
  it('creates notifications for all necessary users for a task', async () => {
    ...
    const user3 = await User.create(createMockUser(15, clinic.id, userRole, 'care@care3.com'), txn);
    const user4 = await User.create(createMockUser(16, clinic.id, userRole, 'care@care4.com'), txn);
